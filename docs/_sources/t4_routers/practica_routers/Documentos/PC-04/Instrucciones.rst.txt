...
* Cliente 1: la tarjeta enp0s3 tiene la IP **1.4.0.2/16**
* Cliente 2: la tarjeta enp0s3 tiene la IP **6.4.0.10/16**
* Izquierda: la tarjeta enp0s3 tiene la IP **1.4.0.1/16**,la tarjeta enp0s8 tiene la IP **2.4.0.1/16**,la tarjeta enp0s9 tiene la IP **3.4.0.1/16**
* Arriba: la tarjeta enp0s3 tiene la IP **Reservada**,la tarjeta enp0s8 tiene la IP **2.4.0.2/16**,la tarjeta enp0s9 tiene la IP **5.4.0.1/16**
* Abajo: la tarjeta enp0s3 tiene la IP **6.4.0.1/16**,la tarjeta enp0s8 tiene la IP **3.4.0.2/16**,la tarjeta enp0s9 tiene la IP **5.4.0.2/16**
...
Izquierda tendría un fichero de ``netplan`` como este::
	...
	network:
	  ...
	  ethernets: 
	    enp0s3:
	      addresses: [1.4.0.1/16]
	    enp0s8:
	      addresses: [2.4.0.1/16]
	    enp0s9:
	      addresses: [3.4.0.1/16]
...
Arriba tendría un fichero de ``netplan`` como este::
	...
	network:
	  ...
	  ethernets: 
	    enp0s8:
	      addresses: [2.4.0.2/16]
	    enp0s9:
	      addresses: [5.4.0.1/16]
...
Abajo tendría un fichero de ``netplan`` como este::
	...
	network:
	  ...
	  ethernets: 
	    enp0s3:
	      addresses: [6.4.0.1/16]
	    enp0s8:
	      addresses: [3.4.0.2/16]
	    enp0s9:
	      addresses: [5.4.0.2/16]
...
En Izquierda podemos ejecutar esto::
	...
	sudo ip route add 6.4.0.0/16 via 3.4.0.2

En Abajo podemos ejecutar esto::

	sudo ip route add 1.4.0.0/16 via 3.4.0.1
...
En Izquierda podemos ejecutar esto::
	...
	sudo ip route add 6.4.0.0/16 via 2.4.0.2

En Arriba podemos ejecutar esto::

	sudo ip route add 1.4.0.0/16 via 2.4.0.1
	sudo ip route add 6.4.0.0/16 via 5.4.0.2

En Abajo podemos ejecutar esto::

	sudo ip route add 1.4.0.0/16 via 5.4.0.1
...
Router izquierda::
	...
	network 1.4.0.0/16 area 1
	network 2.4.0.0/16 area 1
	network 3.4.0.0/16 area 1

Router Arriba::

	network 2.4.0.0/16 area 1
	network 5.4.0.0/16 area 1

Router Abajo::

	network 6.4.0.0/16 area 1
	network 3.4.0.0/16 area 1
	network 5.4.0.0/16 area 1
